...
def search():
	if request.method=='GET':
		...
		firstname=r.hget(aadhar,'firstname')
		street=r.hget(aadhar,'street')
		state=r.hget(aadhar,'state')
		sp=r.hget(aadhar,'sp')
		registerydate=r.hget(aadhar,'registerydate')
		furni=r.hget(aadhar,'furni')
		return render_template('searched.html',pincode=pincode,firstname=firstname,city=city,state=state,street=street,sp=sp,registerydate=registerydate,furni=furni)
